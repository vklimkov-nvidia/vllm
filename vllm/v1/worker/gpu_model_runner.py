...
class GPUModelRunner(
            ...
            if isinstance(model_output, torch.Tensor):
                ...
                if self.model_config.custom_outputs:
                    ...
                    if len(custom_outputs) != len(self.model_config.custom_outputs):
                        raise RuntimeError(
                            f"Expected [{self.model_config.custom_outputs}] "
                            f"but got {len(custom_outputs)} outputs from model"
                        )
                    custom_outputs_flat = {
                        name: val
                        for name, val in zip(
                            self.model_config.custom_outputs, custom_outputs
                        )
                    }
    ...
    def sample_tokens(
        ...
        if custom_outputs_flat:
            ...
            for req_id in req_ids:
                length = scheduler_output.num_scheduled_tokens[req_id]
                custom_outputs_list.append({
                    name: arr_cpu[offset:offset + length]
    ...
    def _zero_kv_cache_blocks(self, block_ids: list[int]) -> None:
        """Zero the KV cache blocks to prevent state contamination when
        blocks are reused by new requests (critical for Mamba layers)."""
        if not block_ids:
            return
                        ...
                        for state_idx, state_tensor in enumerate(ve_cache):
                            if isinstance(state_tensor, torch.Tensor) and state_tensor.numel() > 0:
                                for block_id in block_ids:
                                    if state_tensor.ndim > 0 and block_id < state_tensor.shape[0]:
                                        state_tensor[block_id].zero_()
                                        zeroed_count += 1
                    elif isinstance(ve_cache, torch.Tensor) and ve_cache.numel() > 0:
                        for block_id in block_ids:
                            if ve_cache.ndim > 0 and block_id < ve_cache.shape[0]:
                                ve_cache[block_id].zero_()
                                zeroed_count += 1

        logger.info(
            "Zeroed %d tensors across %d blocks", zeroed_count, len(block_ids)
        )
