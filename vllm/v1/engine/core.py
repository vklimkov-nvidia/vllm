...
class EngineCore:
    ...
    def step_with_batch_queue(
        ...
        if not batch_queue:
            # Queue is empty – nothing scheduled above and no prior batch
            # waiting. Return early instead of crashing on pop().
            return None, False
...
class EngineCoreProc(EngineCore):
    ...
    def has_work(self) -> bool:
        ...
        Uses has_schedulable_requests() rather than has_requests() so that
        requests parked in ``waiting_input`` (waiting for custom inputs via
        ZMQ/SHM) do not prevent the engine from blocking on the input queue.
        For the SHM path we must stay active to poll the channels, hence
        the ``_shm_channels`` check.
        """
        return (
            self.engines_running
            or self.scheduler.has_schedulable_requests()
            or bool(self.batch_queue)
            or bool(self._shm_channels)
        )
    ...
    def _process_input_queue(self):
        ...
        # Skip waiting for custom inputs when the batch queue has in-flight
        # work: the output that would trigger the client's next append hasn't
        # been delivered yet, so waiting would always time out.
        if self.batch_queue:
            return
